...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        page_loaded = exists(wiki_logo_pattern, Settings.FIREFOX_TIMEOUT)
        assert page_loaded is True, 'Page loaded.'

        full_screen()
        try:
            if Settings.is_mac():
                full_screen_on = wait_vanish(MainWindow.UNHOVERED_MAIN_RED_CONTROL, Settings.FIREFOX_TIMEOUT)
            else:
                full_screen_on = wait_vanish(MainWindow.CLOSE_BUTTON, Settings.FIREFOX_TIMEOUT)
        ...
        mouse_scroll_done = scroll_until_pattern_found(wikimedia_logo_pattern, scroll, (-scroll_length,), 20,
                                                       timeout=Settings.FIREFOX_TIMEOUT/8)
